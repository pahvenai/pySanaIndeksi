...
######################
    ...
    def addFromReader(self, wordCount = None):
        ...
        @param wordCount: maximum number of words to be added
        """
        if self.lukija:
    ...
    def find(self, key, output='full', sanitized=False):
        ...
        @param output: the type of output desired, options defined in Tree class
        @type output: string
        @param sanitized: the word is sanitized unless this param is True
        @type sanitized: boolean
        @return: hits returned in the format defined by output
        '''
        if self.wordCount == 0:
            return None
        if not sanitized:
